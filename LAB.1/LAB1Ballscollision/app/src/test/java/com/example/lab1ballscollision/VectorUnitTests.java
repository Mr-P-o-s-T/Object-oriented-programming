...
public class VectorUnitTests {
    ...
    public void addition_isCorrect() {
        ...
        if (Math.abs(c.x - 1.0) >= 0.01) assertFalse(true);
        if (Math.abs(c.y - 1.0) >= 0.01) assertFalse(true);
        if (Math.abs(c.z - 1.0) >= 0.01) assertFalse(true);
    }
    ...
    public void normalisation_isCorrect() {
        ...
        if (Math.abs(a.normalise().length() - 1.0) >= 0.01) assertFalse(true);
    }

    @Test
    public void length_isCorrect() {
        Vector a = new Vector(1.0,1.0,1.0);
        if (Math.abs(a.length() - Math.sqrt(3.0)) >= 0.01) assertFalse(true);
    }

    @Test
    public void multOnScalar_isCorrect() {
        Vector a = new Vector(1.0,2.0,3.0), b = a.multOnScalar(2);
        if (Math.abs(b.x - 2.0) >= 0.01) assertFalse(true);
        if (Math.abs(b.y - 4.0) >= 0.01) assertFalse(true);
        if (Math.abs(b.z - 6.0) >= 0.01) assertFalse(true);
    }

    @Test
    public void scalarCompos_isCorrect() {
        Vector a = new Vector(1.0,2.0,0.0), b = new Vector(-3.0, 3.0, 1.0);
        if (Math.abs(a.scalarCompos(b) - 3.0) >= 0.01) assertFalse(true);
    }

    @Test
    public void vectorCompos_isCorrect() {
        Vector a = new Vector(1.0,2.0,3.0), b = a.multOnScalar(2), c = a.vectorCompos(b);
        if (Math.abs(c.x) >= 0.01) assertFalse(true);
        if (Math.abs(c.y) >= 0.01) assertFalse(true);
        if (Math.abs(c.z) >= 0.01) assertFalse(true);
    }
